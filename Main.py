...
from Networks.DDPG_Networks import DDPG_ActorNetwork, DDPG_CriticNetwork
...
def create_path(activation, fed_comm, alg):
    # path creation based on the algorithm selection
    if (alg == 'opt_non_fair') or (alg == 'opt_fair') or (alg == 'random'):
        folder = 'results_' + alg
        save_path = os.path.join(folder, "Saved_data")
    elif fed_comm:
        folder = 'results_' + 'centralized_' + ('NN' if activation != 'linear' else activation)
        save_path = os.path.join(folder, "Saved_data")
    else:
        folder = 'results_' + 'decentralized_' + ('NN' if activation != 'linear' else activation)
        save_path = os.path.join(folder, "Saved_data")
...
def main(platoon_size, road_length, Headway, CACC, Platoon_speed, safety_distance, n_RB, max_power, BW, N_m, SINR_th,
         ...
         simulation_time, n_train, n_test, V2V_size, MATI, full_data, algorithm, Fed_Comm, train, test, extend, Exp):

    # path creation for saving the data and weights
    folder, save_path = create_path(activation, Fed_Comm, algorithm)
    # Initialization
    ...
    # ---------------------------------------------------------------------------------------------------------------- #
    if algorithm == 'Hybrid':
        action_outage_n = n_RB # RB & Power
        outage_input_size = len(OTTO.get_outage_state(env=env, idx=0))
        actor_net = PDQN_ActorNetwork(actor_lr, outage_input_size, Actor_dims, activation, squashing_func, weight_init,
                                 action_outage_n)
        critic_net = PDQN_CriticNetwork(critic_lr, outage_input_size, Critic_dims, activation, squashing_func, weight_init,
                                   action_outage_n)
    elif algorithm == 'DDPG':
        action_outage_n = 1  # similar for both power & RB
        outage_input_size = len(OTTO.get_outage_state(env=env, idx=0))
        actor_net = DDPG_ActorNetwork(actor_lr, outage_input_size, Actor_dims, activation, squashing_func, weight_init,
                                      action_outage_n)
        critic_net = DDPG_CriticNetwork(critic_lr, outage_input_size, Critic_dims, activation, squashing_func, weight_init,
                                        action_outage_n)
    elif (algorithm == 'opt_fair') or (algorithm == 'opt_non_fair') or (algorithm == 'random'):
        action_outage_n = None  # RB & Power
    ...
    RSU = RSU_server(actor_net, critic_net, n_agents, N_veh, env, HS_CACC, epsilon, gamma, taus, n_train, n_test,
                     ...
                     algorithm, Fed_Comm, save_path, folder, extend, Exp)
    if train:
        RSU.train('train')
    if test:
        RSU.test('test')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--platoon_size", type=int, default=8, help='Platoon Size')
    ...
    parser.add_argument("--safety_distance", type=int, default=50, help='inter platoon safety distance')
    parser.add_argument("--n_RB", type=int, default=3, help='number of resource blocks')
    parser.add_argument("--max_power", type=float, default=30, help='vehicles maximum transmit power')
    parser.add_argument("--BW", type=int, default=180000, help='per RB bandwidth')
    parser.add_argument("--N_m", type=int, default=1, help='Nakagami fading argument')
    parser.add_argument("--SINR_th", type=float, default=3, help='Minimum required Signal to Noise Ratio')
    parser.add_argument("--rate_th", type=float, default=0.5, help='Minimum required data rate')
    ...
    parser.add_argument("--Critic_dims", type=list, default=[32, 16], help='critic nets hidden layers')
    parser.add_argument("--Actor_dims", type=list, default=[32, 16], help='actor nets hidden layers')
    ...
    parser.add_argument("--weight_init", type=str, default='phil_tabor',
    ...
    parser.add_argument("--CAM_size", type=float, default=2000 * 8, help='total CAM message payload (bits)')
    ...
    parser.add_argument("--Algorithm", type=str, default='Hybrid',
                        choices=['Hybrid', 'opt_non_fair', 'opt_fair', 'random'])
    parser.add_argument("--federated_communication", default=True, choices=[True, False])
